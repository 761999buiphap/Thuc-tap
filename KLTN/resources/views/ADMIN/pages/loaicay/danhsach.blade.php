...
@section('noidung')
 ...
 <div class="container-fluid" style="background:linear-gradient( rgb(18, 107, 110),#3dafb3 100%);">
        <div class="col-md-10" style="padding:0%;">
            <h5 style="padding: 5px;"><span style="color: white;">DANH MỤC QUẢN LÝ » </span> <i style="color: #fed000;"><?php foreach($giongcay as $tgc){if($tgc->id == $id){ echo $tgc->ten;}} ?>  </i></h5>
        </div>
        <div class="col-md-1" >
        ...
        <div class="col-md-1">
            <!--them moi giong cay-->
            <a href="/them-loaicay/{{$id}}" class="btn " style="background-color: #EF5350;color: white;margin-top:8%;"><li class="fa fa-plus"></li> Thêm mới</a>
            <div id="myModal_them" class="modal" role="dialog">
                <div class="modal-dialog ">
                    <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title fa fa-plus" style="color: rgb(18, 107, 110);"> Thêm giống <?php foreach($giongcay as $tgc){if($tgc->id == $id){ echo $tgc->ten;}} ?></h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="container">
                        @if(isset($lc_them))
                        <form action="/them-loaicay/{{$id}}"  method="post" enctype="multipart/form-data">
    ...
    <div id="demo" class="collapse container-fluid"  style="padding:0 3%;">
        <form action="/timkiem-loaicay" method="post" class="form-group " style="margin-top:1%;">
        <input type="hidden" name="_token" value="{{csrf_token()}}" />
        <input type="hidden" name="id_giongcay" value="{{$id}}" />
        <div class="row">
            <div class="col-md-4">
                <label for="">Tên giống cây:</label>
                <input class="form-control float-left" type="text" name="ten" value=" " placeholder="Nhập vào tên giống cây" >
            </div>
            <div class="col-md-4">
                <label for="">Trạng thái:</label>
    ...
    <div class="container-fluid" >
        <div class="col-md-3" style="padding: 1% 0;">
            <div class="container-fluid ">
                <div id="danhsach_tintuc" >
                    ...
                    @foreach($giongcay as $value)
                    <a href="/danhsach-loaicay/{{$value->id}}">{{$value->ten}} <span style="float:right;">»</span></a>
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <table class="table table-hover table-bordered table-striped" >
                ...
                foreach($loaicay as $value){ ?>
                  <tr>
                  <td>
                        <a class="btn btn-warning "style="padding:1% 10%;" href="/sua-loaicay/{{$value->id}}/{{$id}}" >Edit</a>
                        <div id="myModal<?php echo $value->id; ?>" class="modal"  role="dialog">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title fa fa-edit"  style="color: rgb(18, 107, 110);"> Sửa loại cây</h4>
                                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                                </div>
                                <div class="modal-body">
                                <div class="container">
                                    @if(isset($lc_sua))      
                                    @foreach($lc_sua as $vl)
                    ...
                    <td >
                        ...
                        <a class="dropdown-item" href="/ngunghoatdong-loaicay/{{$value->id}}/{{$id}}"><li class="fa fa-random"></li> Ngừng hoạt động</a>
                        </div>
                        @endif  
                        ...
                        <div class="dropdown-menu">
                        <a class="dropdown-item" href="/danghoatdong-loaicay/{{$value->id}}/{{$id}}"><li class="fa fa-retweet"></li> Đang hoạt động</a>
                        </div>
                        @endif                
                    </td>
