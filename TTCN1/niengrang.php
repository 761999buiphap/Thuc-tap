...
<body style='font-family:arial;'>
    ...
    ?>
    <div class="div2">
        <p style='margin-left:1%;'>DỊCH VỤ >> <span style="color:blue;">Niềng răng</span> </p>
        <div class=div2form>
        <?php 
            $truyvansum="SELECT COUNT(mabs) AS dem FROM niengrang WHERE mabs='$tentk'";
    ...
    <div class=div3>
        <?php 
            if(isset($_POST['luu']))
            {
                ...
                $loaimaccai=$_POST['loaimaccai'];
                $giaidoannieng=$_POST['giaidoannieng'];
                $denghi=$_POST['denghi'];
                $ghichu=$_POST['ghichu'];
                if(!empty($mabn&&$ngay&&$dungcu&&$denghi&&$ghichu))
                {
                    $truyvanluu="INSERT INTO niengrang(mabn,mabs,ngay,tinhtrang,loaimaccai,giaidoannieng,dungcu,denghi,ghichu) VALUES('$mabn','$tentk','$ngay','$tinhtrang','$loaimaccai','$giaidoannieng','$dungcu','$denghi','$ghichu')";
        ...
        <div class="div3_1">
            <?php 
                if(isset($_POST['mabn'])||isset($_POST['kham']))
                {
                    ...
                    echo "<br><span>Bác sĩ thực hiện: </span>"  ."<span style='color:green;'>".$row2['hoten'] ."</span>" ."<span style='margin-left:40px;font-weight:blod;'>Chỉ định: </span><span style='color:green;'> Niềng răng</span>";
                    echo "<br><br><br><span>Ngày: </span>"  ."<span style='margin-left:82px;font-weight:normal;'>".date('d-m-Y') ."</span>"; 
                    echo "<br><span>Tình trạng:</span><select style='font-size:17px;margin:5px;height:30px;outline:none;margin-left:50px;font-weight:normal; width:25%;' id=tinhtrang name='tinhtrang'> <option value='Khớp cắn sâu'>Khớp cắn ngược</option><option value='Khớp cắn chéo'>Khớp cắn chéo</option><option value='Răng thưa'>Răng thưa</option><option value='Khớp cắn hở'>Khớp cắn hở</option><option value='Răng quá chen chúc'>Răng quá chen chúc</option><option value='Răng khấp khểnh'>Răng khấp khểnh</option></select>";
                    echo "<br><span>Loại mắc cài:</span><select style='font-size:17px;margin:5px;height:30px;outline:none;margin-left:34px;font-weight:normal; width:25%;' id=loaimaccai name='loaimaccai'><option value=' Mắc cài kim loại thường (buộc chun)'> Mắc cài kim loại thường (buộc chun)</option><option value='Mắc cài kim loại tự buộc'>Mắc cài kim loại tự buộc</option><option value=' Mắc cài pha lê'> Mắc cài pha lê</option><option value='Mắc cài pha lê tự buộc'>Mắc cài pha lê tự buộc</option><option value='Niềng khay trong suốt Invisalign'>Niềng khay trong suốt Invisalign</option></select>";
                    echo "<br><span>Giai đoạn niềng:</span><select style='font-size:17px;margin:5px;height:30px;outline:none;margin-left:10px;font-weight:normal; width:25%;' id=giaidoannieng name='giaidoannieng'> <option value='None'>None</option><option value='Sau 3 tháng'>Sau 3 tháng</option><option value='Sau 6 tháng'>Sau 6 tháng</option><option value='Sau 9 tháng'>Sau 9 tháng</option><option value='Sau 15 tháng'>Sau 15 tháng</option><option value='Kết thúc quá trình'>Kết thúc quá trình</option></select>";
        ...
        <div class='div3_2'>
            ...
            <?php
                $ngay=date('Y-m-d');
                $truyvan="SELECT * FROM thuthuat WHERE thuthuatdieutri='Niềng răng' and ngaythuchien='$ngay'";
                $lienket=mysqli_query($CONN,$truyvan);
                if(mysqli_num_rows($lienket)>0)
                {
                        while($row=mysqli_fetch_assoc($lienket))
                        {
                            $truyvan1="SELECT * FROM niengrang WHERE ngay='$ngay' and mabn='$row[mabn]'";
                            $lienket1=mysqli_query($CONN,$truyvan1);
        ...
        <h4 style='margin-top:30px;color:white;text-align:center;'>Danh sách lịch hẹn</h4>
            ...
            <?php
                $truyvan2="SELECT * FROM lichhen WHERE noidung='Niềng răng' and ngayhen='$ngay'";
                $lienket2=mysqli_query($CONN,$truyvan2);
                if(mysqli_num_rows($lienket2)>0)
                {
                    ...
                    {
                        $i=1;
                        $truyvan1="SELECT * FROM niengrang WHERE ngay='$ngay' and mabn='$row2[mabn]'";
                        $lienket1=mysqli_query($CONN,$truyvan1);
                        if(mysqli_num_rows($lienket1)<=0)
                        {
