...
<body>
    ...
    <?php
        ...
        ?>
    <div class="div2">
        <p>Dữ liệu >> <span style="color:blue;">Hồ sơ bác sĩ >></span> Thêm
        </p>
    </div>
<?php 
 $mabs1=$tenbs1=$ngaysinh1=$gioitinh1=$sdt1=$diachi1=$email1=$tieusubenh1=$ttk1=$mk1=$luongcb1=$chucvu1='';
 ?>    
<?php
if(isset($_POST['luu']))
{
        ...
        if(!empty($chucvu)&&!empty($luongcb)&&!empty($ttk)&&!empty($mk)&&!empty($mabs)&& !empty($tenbs)&& !empty($ngaysinh)&& !empty($_POST['gioitinh'])&& !empty($sdt)&& !empty($email)&&!empty($ngayden)&&!empty($diachi))
        {
             $truyvan1="SELECT * FROM hosobacsy WHERE mabs='$mabs' OR sdt='$sdt'";
             $lienket1=mysqli_query($CONN,$truyvan1);
             $truyvantk="SELECT * FROM taikhoanbacsy WHERE tentk='$mabs'";
            ...
            {
                $truyvanluu= "INSERT INTO hosobacsy(mabs,hoten,ngaysinh,gioitinh,sdt,email,diachi,ngayden,chucvu,luongcb) VALUES('$mabs','$tenbs','$ngaysinh','$gioitinh','$sdt','$email','$diachi','$ngayden','$chucvu','$luongcb')";
                $lienketluu = mysqli_query($CONN,$truyvanluu)or die("sai1");
                $truyvantk1= "INSERT INTO taikhoanbacsy(tentk,matkhau) VALUES('$ttk','$mk')";
                $lienkettk1 = mysqli_query($CONN,$truyvantk1)or die("sai2");
                echo "<h3 style=' margin-left:40%;color:red;'>< Thêm thành công ></h3>";
                $truyvan2="SELECT * FROM hosobacsy WHERE mabs='$mabs' OR sdt='$sdt'";
                $lienket2=mysqli_query($CONN,$truyvan2);
                $row2=mysqli_fetch_assoc($lienket2);
                $truyvantk2="SELECT * FROM taikhoanbacsy WHERE tentk='$mabs'";
                $lienkettk2=mysqli_query($CONN,$truyvantk2)or die("sai...");
                $rowtk2=mysqli_num_rows($lienkettk2);
                $mabs1=$row2['mabs'];
                $tenbs1=$row2['hoten'];
                $nsbs1=$row2['ngaysinh'];
                $ngaysinh1=date('d-m-Y',strtotime($nsbs1));
...
<div style="position:relative;width:100%;height:400px;">
    ...
    <div class="div4" >
        ...
        <input style='margin-left:125px;' type='text' name='mabs' value="<?php echo $mabs1; ?>"><br>
        <lable for="tenbs">Họ tên: </lable>
        <input style="width:60%;margin-left:100px;" type="text" name="tenbs" value="<?php echo $tenbs1; ?>"><br>
        <lable for="nsbn">Ngày sinh:</lable>
        <input style="margin-left:75px;" type="text" name="nsbs" value="<?php echo $ngaysinh1; ?>">
