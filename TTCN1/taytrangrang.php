...
<body style='font-family:arial;'>
    ...
    ?>
    <div class="div2">
        <p style='margin-left:1%;'>DỊCH VỤ >> <span style="color:blue;">Lấy cao răng</span> </p>
        <div class=div2form>
        <?php 
            $truyvansum="SELECT COUNT(mabs) AS dem FROM taytrangrang WHERE mabs='$tentk'";
    ...
    <div class=div3>
        ...
        //Lưu
            if(isset($_POST['luu']))
            {
                ...
                {
                    $truyvanluu="INSERT INTO taytrangrang(mabn,mabs,ngay,dungcu,denghi,ghichu) VALUES('$mabn','$tentk','$ngay','$dungcu','$denghi','$ghichu')";
        ...
        <div class="div3_1">
            <?php 
                if(isset($_POST['mabn'])||isset($_POST['kham']))
                {
                    ...
                    echo "<br><span>Bác sĩ thực hiện: </span>"  ."<span style='color:green;'>".$row2['hoten'] ."</span>" ."<span style='margin-left:40px;font-weight:blod;'>Chỉ định: </span><span style='color:green;'> Tẩy trắng răng laser</span>";
                    echo "<br><br><br><span>Ngày: </span>"  ."<span style='margin-left:75px;font-weight:normal;'>".date('d-m-Y') ."</span>" ."<br><span>Dụng cụ: <input style='margin:5px;height:55px;outline:none;margin-left:50px;font-weight:normal; width:73%;' type=text name=dungcu value=''> </span>";
        ...
        <div class='div3_2'>
            ...
            <?php
                $ngay=date('Y-m-d');
                $truyvan="SELECT * FROM thuthuat WHERE thuthuatdieutri='Tẩy trắng răng' and ngaythuchien='$ngay'";
                $lienket=mysqli_query($CONN,$truyvan);
                if(mysqli_num_rows($lienket)>0)
                {
                        while($row=mysqli_fetch_assoc($lienket))
                        {
                            $truyvan1="SELECT * FROM taytrangrang WHERE ngay='$ngay' and mabn='$row[mabn]'";
                            $lienket1=mysqli_query($CONN,$truyvan1);
        ...
        <h4 style='margin-top:30px;color:white;text-align:center;'>Danh sách lịch hẹn</h4>
            ...
            <?php
                $truyvan2="SELECT * FROM lichhen WHERE noidung='Tẩy trắng răng' and ngayhen='$ngay'";
                $lienket2=mysqli_query($CONN,$truyvan2);
                if(mysqli_num_rows($lienket2)>0)
                {
                    ...
                    {
                        $i=1;
                        $truyvan1="SELECT * FROM taytrangrang WHERE ngay='$ngay' and mabn='$row2[mabn]'";
                        $lienket1=mysqli_query($CONN,$truyvan1);
                        if(mysqli_num_rows($lienket1)<=0)
                        {
                            echo "<tr>";
