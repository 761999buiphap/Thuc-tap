...
<body style='font-family:arial;'>
    ...
    ?>
    <div class="div2">
        <p style='margin-left:1%;'>DỊCH VỤ >> <span style="color:blue;">Răng sứ thẩm mỹ</span> </p>
        <div class=div2form>
        <?php 
            $truyvansum="SELECT COUNT(mabs) AS dem FROM rangsuthammy WHERE mabs='$tentk'";
    ...
    <div class=div3>
        <?php 
            if(isset($_POST['luu']))
            {
                ...
                $ngay=date("Y-m-d");
                $dungcu=$_POST['dungcu'];
                $loairang=$_POST['loairang'];

                $truyvan2="SELECT mara FROM rang WHERE tenrang='$loairang'";
                ...
                {
                    $truyvanluu="INSERT INTO rangsuthammy(mabn,mabs,ngay,mara,soluongrang,dungcu,denghi,ghichu) VALUES('$mabn','$tentk','$ngay','$loairang','$soluongrang','$dungcu','$denghi','$ghichu')";
        ...
        <div class="div3_1">
            <?php 
                if(isset($_POST['mabn'])||isset($_POST['kham']))
                {
                    ...
                    echo "<br><span>Bác sĩ thực hiện: </span>"  ."<span style='color:green;'>".$row2['hoten'] ."</span>" ."<span style='margin-left:40px;font-weight:blod;'>Chỉ định: </span><span style='color:green;'> Răng sứ thẩm mỹ</span>";
                    echo "<br><br><br><span>Ngày: </span>"  ."<span style='margin-left:75px;font-weight:normal;'>".date('d-m-Y') ."</span>" ."<br><span>Dụng cụ: <input style='margin-left:50px;font-weight:normal; width:73%;height:55px;outline:none;' type=text name=dungcu value=''> </span>";
                    echo "<br><span>Loại răng:</span><select style='font-size:17px;margin:5px;height:30px;outline:none;margin-left:47px;font-weight:normal; width:25%;' id=loairang name='loairang'> <option value='Răng sứ venus'>Răng sứ venus</option><option value='Răng sứ emaxzic'>Răng sứ emaxzic</option><option value='Răng sứ cercon'>Răng sứ cercon</option><option value='Răng sứ bio paris'>Răng sứ bio paris</option><option value='Răng sứ lisi'>Răng sứ lisi</option><option value='Răng sứ 4s'>Răng sứ 4s</option><option value='Răng sứ kim cương'>Răng sứ kim cương</option><option value='Veneer sứ emax'>Veneer sứ emax</option><option value='Veneer ultra thin cao cấp'>Veneer ultra thin cao cấp</option></select>";
                    echo "<span>Số lượng răng: <input style='margin:5px;height:30px;outline:none;margin-left:4px;font-weight:normal; width:10%;' type=text name=soluongrang value=''> </span>";
                    echo "<br><span>Đề nghị: <input style='margin:5px;height:55px;outline:none;margin-left:55px;font-weight:normal; width:73%;' type=text name=denghi value=''> </span>" ."<br><span>Ghi chú: <input style='margin:5px;height:55px;outline:none;margin-left:55px;font-weight:normal; width:73%;' type=text name=ghichu value=''> </span>";
                ...
                else
                {
                    ...
                    echo "<br><span>Bác sĩ thực hiện: </span>"  ."<span style='color:green;'>".$row2['hoten'] ."</span>" ."<span style='margin-left:40px;font-weight:blod;'>Chỉ định: </span><span style='color:green;'> Răng sứ thẩm mỹ</span>";
                    echo "<br><br><br><span>Ngày: </span>"  ."<span style='margin-left:75px;font-weight:normal;'>".date('d-m-Y') ."</span>" ."<br><span>Dụng cụ: <input style='margin-left:50px;font-weight:normal; width:73%;height:55px;outline:none;' type=text name=dungcu value=''> </span>";
                    echo "<br><span>Loại răng:</span><select style='font-size:17px;margin:5px;height:30px;outline:none;margin-left:47px;font-weight:normal; width:25%;' id=tenrang name='loairang'> <option value='Răng sứ venus'>Răng sứ venus</option><option value='Răng sứ emaxzic'>Răng sứ emaxzic</option><option value='Răng sứ cercon'>Răng sứ cercon</option><option value='Răng sứ bio paris'>Răng sứ bio paris</option><option value='Răng sứ lisi'>Răng sứ lisi</option><option value='Răng sứ 4s'>Răng sứ 4s</option><option value='Răng sứ kim cương'>Răng sứ kim cương</option><option value='Veneer sứ emax'>Veneer sứ emax</option><option value='Veneer ultra thin cao cấp'>Veneer ultra thin cao cấp</option></select>";
                    echo "<span>Số lượng răng: <input style='margin:5px;height:30px;outline:none;margin-left:4px;font-weight:normal; width:10%;' type=text name=soluongrang value=''> </span>";
                    echo "<br><span>Đề nghị: <input style='margin:5px;height:55px;outline:none;margin-left:55px;font-weight:normal; width:73%;' type=text name=denghi value=''> </span>" ."<br><span>Ghi chú: <input style='margin:5px;height:55px;outline:none;margin-left:55px;font-weight:normal; width:73%;' type=text name=ghichu value=''> </span>";
                    echo "<br><input onclick='window.print();' style='padding:5px 20px;background-color:green;margin:5% 0px 0px 46%;color:white;' type=submit name=luu value='Print'></form>";
                }
        ...
        <div class='div3_2'>
            ...
            <?php
                $ngay=date('Y-m-d');
                $truyvan="SELECT * FROM thuthuat WHERE thuthuatdieutri='Răng sứ thẩm mỹ' and ngaythuchien='$ngay'";
                $lienket=mysqli_query($CONN,$truyvan);
                if(mysqli_num_rows($lienket)>0)
                {
                        while($row=mysqli_fetch_assoc($lienket))
                        {
                            $truyvan1="SELECT * FROM rangsuthammy WHERE ngay='$ngay' and mabn='$row[mabn]'";
                            $lienket1=mysqli_query($CONN,$truyvan1);
        ...
        <h4 style='margin-top:30px;color:white;text-align:center;'>Danh sách lịch hẹn</h4>
            ...
            <?php
                $truyvan2="SELECT * FROM lichhen WHERE noidung='Răng sứ thẩm mỹ' and ngayhen='$ngay'";
                $lienket2=mysqli_query($CONN,$truyvan2);
                if(mysqli_num_rows($lienket2)>0)
                {
                    ...
                    while($row2=mysqli_fetch_assoc($lienket2))
                    {
                        ...
                        echo "<td style='background-color:'>$i</td>";
